<template>
  <div class="flex h-full w-full bg-bg-base">
    ...
    <div class="px-6 py-4 border-b border-border-default flex items-center justify-between">
      <h1 class="text-2xl font-bold text-text-default">{{ t('server.select.yourServers') }}</h1>
    ...
    <div class="flex-1 p-6 overflow-y-auto">
      ...
      <div v-else-if="appStore.servers.length === 0"
        ...
        <h2 class="text-xl font-semibold text-text-default mb-2">{{ t('server.select.emptyTitle') }}</h2>
        <p class="text-text-muted mb-6">{{ t('server.select.emptyDescription') }}</p>
        <button
            class="btn-primary flex items-center gap-2"
            @click="openCreateModal"
        >
          <Plus class="w-5 h-5"/>
          {{ t('server.select.addServer') }}
        </button>
      </div>
          ...
          <div class="flex items-center gap-4 mb-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-sm text-text-muted">
                {{ server.memberCount }} {{ t(server.memberCount === 1 ? 'server.explore.member' : 'server.explore.members') }}
              </p>
            </div>
          </div>
